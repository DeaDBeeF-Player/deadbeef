...
static const char *aa_fields_cached[num_aa_fields] = { NULL, };
static const char *a_fields_cached[num_a_fields] = { NULL, };
static const char *alb_fields_cached[num_alb_fields] = { NULL, };
static const char *title_fields_cached[num_title_fields] = { NULL, };
static const char *genre_fields_cached[num_genre_fields] = { NULL, };
...
typedef struct {
    ...
    /// variable data, for $put/$puts/$get
    ddb_tf_var_t *vars;
...
static void
_tf_var_deinit (ddb_tf_var_t *var) {
    free (var->key);
    free (var->value);
}

static const char *
_tf_var_get_value_for_key (ddb_tf_context_int_t *ctx, const char *key) {
    ddb_tf_var_t *var = ctx->vars;
    while (var != NULL) {
        if (!u8_strcasecmp (key, var->key)) {
            return var->value;
        }
        var = var->next;
    }
...
static void
_tf_vars_set_value_for_key (ddb_tf_context_int_t *ctx, const char *key, const char *value) {
    // check if key is already defined, if so, overwrite it
    ddb_tf_var_t *cur = ctx->vars;
    while (cur != NULL) {
...
_tf_vars_free (ddb_tf_context_int_t *ctx) {
    while (ctx->vars) {
        ddb_tf_var_t *next = ctx->vars->next;
        ddb_tf_var_t *var = ctx->vars;
        _tf_var_deinit (var);
        free (var);
        ctx->vars = next;
...
tf_func_get (ddb_tf_context_t *ctx, int argc, const uint16_t *arglens, const char *args, char *out, int outlen, int fail_on_undef) {
    ...
    len = u8_strnbcpy (out, value, outlen);
...
_tf_get_combined_value_cached_key (playItem_t *it, const char *key, int *needs_free, int item_index, int is_cached_key, int needs_mutex_lock) {
    ...
    if (!is_cached_key) {
        ...
        meta = pl_meta_for_cached_key(it, key, needs_mutex_lock);
    }
...
tf_compile_field (tf_compiler_t *c) {
    ...
    *(c->o++) = 0;
    ...
    char *field = alloca(len+1);
    memcpy(field, fstart, len);
    field[len] = 0;

    // detect special fields, and upgrade to optimized field block
    if (!strcmp(field, aa_fields[0])) {
        c->o = type_ptr;
        *(c->o++) = 6;
        *(c->o++) = 0;
    } else if (!strcmp(field, a_fields[0])) {
        c->o = type_ptr;
        *(c->o++) = 6;
        *(c->o++) = 1;
    } else if (!strcmp(field, alb_fields[0])) {
        c->o = type_ptr;
        *(c->o++) = 6;
        *(c->o++) = 2;
    } else if (!strcmp(field, title_fields[0])) {
        c->o = type_ptr;
        *(c->o++) = 6;
        *(c->o++) = 3;
    } else if (!strcmp(field, genre_fields[0])) {
        c->o = type_ptr;
        *(c->o++) = 6;
        *(c->o++) = 4;
    } else if (!strcmp(field, trknr_fields[0])) {
        c->o = type_ptr;
        *(c->o++) = 6;
        *(c->o++) = 5;
    } else if (!strcmp(field, disc_fields[0])) {
        c->o = type_ptr;
        *(c->o++) = 6;
        *(c->o++) = 6;
    } else {
