...
csid_init (DB_fileinfo_t *_info, DB_playItem_t *it) {
    ...
    info->sidplay = new sidplayfp;
    info->resid = new ReSIDfpBuilder ("wtf");
    ...
    SidConfig conf = info->sidplay->config();
    conf.frequency = samplerate;
    conf.samplingMethod = SidConfig::INTERPOLATE;
    conf.fastSampling = false;

    conf.playback = deadbeef->conf_get_int ("sid.mono", 0) ? SidConfig::MONO : SidConfig::STEREO;
    conf.sidEmulation = info->resid;
    info->sidplay->config (conf);
    info->sidplay->load (info->tune);

    _info->plugin = &sid_plugin;
    _info->fmt.channels = conf.playback == SidConfig::STEREO ? 2 : 1;
    _info->fmt.bps = bps;
...
csid_read (DB_fileinfo_t *_info, char *bytes, int size) {
    ...
    int rd = info->sidplay->play ((short *)bytes, size/2);

    int samplesize = (_info->fmt.bps>>3) * _info->fmt.channels;

    _info->readpos += rd * 2 / samplesize / (float)_info->fmt.samplerate;
...
csid_seek (DB_fileinfo_t *_info, float time) {
    ...
    while (samples > 0) {
        ...
        int done = info->sidplay->play ((short *)buffer, n);
        if (done < n) {
...
//    deadbeef->md5_finish (&md5, sig);
    ...
    for (int s = 0; s < tunes; s++) {
        ...
        if (tune->selectSong (s+1)) {
            ...
            const SidTuneInfo *sidinfo = tune->getInfo ();
