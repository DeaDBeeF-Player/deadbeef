...
ddb_tabstrip_realize (GtkWidget *widget) {
    ...
    if (!gtk_widget_get_has_window (widget)) {
        ...
        attributes.event_mask |= GDK_EXPOSURE_MASK | GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK |
                                 GDK_POINTER_MOTION_MASK | GDK_KEY_PRESS_MASK | GDK_LEAVE_NOTIFY_MASK;
    ...
    GtkTargetEntry entries[] = {
        ...
        { .target = TARGET_PLAYITEM_POINTERS,        .flags = GTK_TARGET_SAME_APP, .info = INFO_TARGET_PLAYITEM_POINTERS},
    };
    gtk_drag_dest_set (
        widget,
...
static void
ddb_tabstrip_size_allocate (GtkWidget *widget, GtkAllocation *allocation) {
...
gboolean
on_tabstrip_scroll_event (GtkWidget *widget, GdkEventScroll *event);

gboolean
on_tabstrip_button_press_event (GtkWidget *widget, GdkEventButton *event);

gboolean
on_tabstrip_button_release_event (GtkWidget *widget, GdkEventButton *event);

gboolean
on_tabstrip_configure_event (GtkWidget *widget, GdkEventConfigure *event);

gboolean
on_tabstrip_draw (GtkWidget *widget, cairo_t *cr);

gboolean
on_tabstrip_expose_event (GtkWidget *widget, GdkEventExpose *event);

gboolean
on_tabstrip_motion_notify_event (GtkWidget *widget, GdkEventMotion *event);

gboolean
on_tabstrip_drag_motion_event (GtkWidget *widget, GdkDragContext *drag_context, gint x, gint y, guint time);

gboolean
on_tabstrip_drag_drop (GtkWidget *widget, GdkDragContext *drag_context, gint x, gint y, guint time);

void
on_tabstrip_drag_data_received (
    GtkWidget *widget,
...
void
on_tabstrip_drag_leave (GtkWidget *widget, GdkDragContext *drag_context, guint time);

void
on_tabstrip_drag_end (GtkWidget *widget, GdkDragContext *drag_context);

gboolean
on_tabstrip_key_press_event (GtkWidget *widget, GdkEventKey *event);
static int
...
ddb_tabstrip_init (DdbTabStrip *tabstrip) {
    ...
    gtk_widget_set_events (
        GTK_WIDGET (tabstrip),
        gtk_widget_get_events (GTK_WIDGET (tabstrip)) | GDK_SCROLL_MASK | GDK_KEY_PRESS_MASK);
#endif
}
...
ddb_tabstrip_draw_tab (
    ...
    cairo_set_source_rgb (
        cr,
        clr_outer_frame.red / 65535.f,
        clr_outer_frame.green / 65535.f,
        clr_outer_frame.blue / 65535.0);
    cairo_draw_lines (cr, points_frame1, sizeof (points_frame1) / sizeof (coord_t));
    cairo_stroke (cr);
    cairo_set_source_rgb (
        cr,
        clr_inner_frame.red / 65535.f,
        clr_inner_frame.green / 65535.f,
        clr_inner_frame.blue / 65535.0);
    ...
    for (idx = 0; idx < cnt; idx++) {
        ...
        if (idx != tab_selected) {
            ...
            draw_text_custom (
                ...
                x + text_left_padding,
                y - text_vert_offset,
                w - (text_left_padding + text_right_padding - 1),
                0,
                DDB_TABSTRIP_FONT,
                bold,
                italic,
...
#    endif
    ...
    // Draw add playlist button
    {
        ...
        float fg[3] = { (float)textcolor.red / 0xffff,
                        (float)textcolor.green / 0xffff,
                        (float)textcolor.blue / 0xffff };
#if !GTK_CHECK_VERSION(3, 0, 0)
        gdk_gc_set_rgb_fg_color (gc, &clr);
        ...
        draw_text_custom (
            &ts->drawctx,
            tabarea_width,
            playlist_button_border,
            add_playlist_btn_width,
            2,
            DDB_TABSTRIP_FONT,
            TRUE,
            FALSE,
    ...
    if (TEST_LEFT_CLICK (event)) {
        ...
        if (need_arrows) {
            if (event->x < arrow_widget_width) {
                ...
                event->x >= a.width - (arrow_widget_width + add_playlist_btn_width) &&
                event->x < a.width - add_playlist_btn_width) {
                if (event->type == GDK_BUTTON_PRESS) {
                    tabstrip_scroll_right (ts);
                    ts->scroll_direction = 1;
                    ts->scroll_timer = g_timeout_add (300, tabstrip_scroll_cb, ts);
...
on_tabstrip_key_press_event (GtkWidget *widget, GdkEventKey *event) {
    ...
    default:
        return on_mainwin_key_press_event (widget, event, NULL);
    }
    return TRUE;
}
