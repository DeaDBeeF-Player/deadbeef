...
} w_selproperties_t, w_selmetadata_t;
...
////// selection metadata widget

gboolean
fill_selmetadata_cb (gpointer data) {
    w_selmetadata_t *w = data;
    DB_playItem_t **tracks = NULL;
    if (w->refresh_timeout) {
        g_source_remove (w->refresh_timeout);
        w->refresh_timeout = 0;
...
static void
selmetadata_selection_changed (gpointer user_data)
{
    w_selmetadata_t *selmeta_w = user_data;
    if (selmeta_w->refresh_timeout) {
        g_source_remove (selmeta_w->refresh_timeout);
        selmeta_w->refresh_timeout = 0;
    }
    selmeta_w->refresh_timeout = g_timeout_add_full(G_PRIORITY_DEFAULT_IDLE, 10, fill_selmetadata_cb, user_data, NULL);
}

static int
selmetadata_message (ddb_gtkui_widget_t *w, uint32_t id, uintptr_t ctx, uint32_t p1, uint32_t p2) {
    switch (id) {
    case DB_EV_TRACKINFOCHANGED:
    case DB_EV_PLAYLISTCHANGED:
        if (p1 == DDB_PLAYLIST_CHANGE_CONTENT || p1 == DDB_PLAYLIST_CHANGE_SELECTION) {
            selmetadata_selection_changed (w);
        }
        break;
    case DB_EV_PLAYLISTSWITCHED:
        selmetadata_selection_changed (w);
...
static void
w_selmetadata_init (struct ddb_gtkui_widget_s *widget) {
    w_selmetadata_t *w = (w_selmetadata_t *)widget;
    w->refresh_timeout = 0;
    fill_selmetadata_cb (widget);
}

static void
on_selmetadata_showheaders_toggled (GtkCheckMenuItem *checkmenuitem, gpointer          user_data) {
    w_selmetadata_t *w = user_data;
    int showheaders = gtk_check_menu_item_get_active (GTK_CHECK_MENU_ITEM (checkmenuitem));
    deadbeef->conf_set_int ("gtkui.selection_metadata.show_headers", showheaders);
    gtk_tree_view_set_headers_visible (GTK_TREE_VIEW (w->tree), showheaders);
}

static void
w_selmetadata_initmenu (struct ddb_gtkui_widget_s *w, GtkWidget *menu) {
    GtkWidget *item;
    item = gtk_check_menu_item_new_with_mnemonic (_("Show Column Headers"));
    gtk_widget_show (item);
    int showheaders = deadbeef->conf_get_int ("gtkui.selection_metadata.show_headers", 1);
    gtk_check_menu_item_set_active (GTK_CHECK_MENU_ITEM (item), showheaders);
    gtk_container_add (GTK_CONTAINER (menu), item);
    g_signal_connect ((gpointer) item, "toggled",
            G_CALLBACK (on_selmetadata_showheaders_toggled),
            w);
}

ddb_gtkui_widget_t *
w_selmetadata_create (void) {
    w_selmetadata_t *w = malloc (sizeof (w_selmetadata_t));
    memset (w, 0, sizeof (w_selmetadata_t));

    w->base.widget = gtk_event_box_new ();
    w->base.init = w_selmetadata_init;
    w->base.message = selmetadata_message;
    w->base.initmenu = w_selmetadata_initmenu;
