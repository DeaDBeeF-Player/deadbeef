...
#define trace(...) \
    { deadbeef->log_detailed (&plugin.decoder.plugin, 0, __VA_ARGS__); }
...
static SF_VIRTUAL_IO vfs = { .get_filelen = sf_vfs_get_filelen,
                             .seek = sf_vfs_seek,
                             .read = sf_vfs_read,
                             .write = sf_vfs_write,
                             .tell = sf_vfs_tell };
...
sndfile_read (DB_fileinfo_t *_info, char *bytes, int size) {
    ...
    if (size / samplesize + info->currentsample > info->endsample) {
        ...
        trace (
            "sndfile: size truncated to %d bytes, cursample=%d, endsample=%d\n",
            size,
            info->currentsample,
            info->endsample);
        if (size <= 0) {
        ...
        if (info->sf_format == SF_FORMAT_PCM_U8) {
            ...
            case 32: {
                ...
                for (int i = 0; i < n / 4; i++, data++) {
                    *data = ((*data & 0xff) << 24) | ((*data & 0xff00) << 8) | ((*data & 0xff0000) >> 8) |
                            ((*data & 0xff0000) >> 24);
                }
            } break;
            }
...
sndfile_insert (ddb_playlist_t *plt, DB_playItem_t *after, const char *fname) {
    ...
    for (int i = 0; map[i].name != NULL; i++) {
        if (map[i].identifier == identifier) {
            format_name = map[i].name;
        }
    }

    deadbeef->pl_add_meta (it, ":FILETYPE", format_name);
    ...
    // sndfile subformats
    int subformat_ids[] = { SF_FORMAT_PCM_S8,    SF_FORMAT_PCM_16,    SF_FORMAT_PCM_24,   SF_FORMAT_PCM_32,
                            ...
                            SF_FORMAT_DPCM_8,    SF_FORMAT_DPCM_16,   SF_FORMAT_VORBIS,   SF_FORMAT_ALAC_16,
                            SF_FORMAT_ALAC_20,   SF_FORMAT_ALAC_24,   SF_FORMAT_ALAC_32,  -1 };

    const char *subformats[] = {
        "PCM_S8",    "PCM_16",   "PCM_24", "PCM_32",    "PCM_U8",  "FLOAT",   "DOUBLE",  "ULAW",    "ALAW",
        "IMA_ADPCM", "MS_ADPCM", "GSM610", "VOX_ADPCM", "G721_32", "G723_24", "G723_40", "DWVW_12", "DWVW_16",
        "DWVW_24",   "DWVW_N",   "DPCM_8", "DPCM_16",   "VORBIS",  "ALAC_16", "ALAC_20", "ALAC_24", "ALAC_32",
    };
